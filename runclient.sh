...
# runclient.sh - Installs dependencies and runs Server and Client_main concurrently
...
# Setup Client_main .env
if [ ! -f "./Client_main/.env" ]; then
    echo "Creating Client_main/.env from example..."
...
if [ ! -d "./Client_main/node_modules" ]; then
    echo "Installing Client_main dependencies..."
    (cd Client_main && npm install)
fi
...
echo " Client_main: http://localhost:8080"
